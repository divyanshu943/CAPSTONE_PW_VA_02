...
shape_predictor = "E:/project/Mouse_Cursor_Control_Handsfree/model/shape_predictor_68_face_landmarks.dat"
...
while True:
    # Grabing the frame from the threaded video file stream, resize
    ...
    # using the Loop over the face detections
    if len(rects) > 0:
        rect = rects[0]
    else:
    ...
    # for some reason the frame is flipped ,so left is right, right is left.
    temp = leftEye
    leftEye = rightEye
    rightEye = temp
    ...
    if diff_ear > WINK_AR_DIFF_THRESH:
        ...
        if ear <= EYE_AR_THRESH:
            ...
            # elif EYE_COUNTER == 3:
            #     j="arbaz1"
            #     client.publish('raspberry/topic', payload=j, qos=0, retain=False)
            #     print(f"send {j} to raspberry/topic")
            #     time.sleep(1)
    ...
    if INPUT_MODE:
       # cv2.putText(frame, "COUNTING!", (2,20), cv2.FONT_HERSHEY_PLAIN, 0.7, RED_COLOR, 2)
